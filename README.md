...
**实测结果**（6 轮区分器 / 64 对明文 / 20 次试验）：

| 指标 | 本复现（8 轮攻击） | 论文表 3（9 轮攻击） |
|---|---|---|
| 平均密钥排名 | **0.5** | 52.1 |
| 中位密钥排名 | **0** | 1.0 |
| 第一名比例 | **70%** | 35.8%（成功率） |
...
> ⚠️ **不能直接与论文比较**：本复现攻击 8 轮（论文 9 轮），且区分器更强（6 轮 0.7882 vs 7 轮 0.616）。攻击难度随轮数指数上升，所以这组数字更好是预期的。严格对照需训练 7 轮区分器后攻击 9 轮。
