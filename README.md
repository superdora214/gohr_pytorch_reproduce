...
用 **PyTorch 从零复现** [Gohr, "Improving Attacks on Round-Reduced Speck32/64 using Deep Learning", CRYPTO 2019] 中的**神经区分器（Neural Distinguisher）**，并打通了从区分器到**密钥恢复攻击**的完整链路。

本项目在理解 Gohr 论文及其官方实现（[agohr/deep_speck](https://github.com/agohr/deep_speck)，numpy/Keras 版）的基础上，用 PyTorch 重写为 **GPU 向量化**版本，支持批量生成训练数据、快速训练、断点续训、实时监控与密钥恢复攻击。

**当前进度**：6 轮区分器达 0.7882（论文 0.788）、9 轮密钥恢复攻击平均排名 41.8（论文 52.1）——**核心实验已完整复现**。
...
| 轮数 | 经典 DDT 基准 (D) | 论文神经目标 (N) | 本复现 | 训练量 |
|------|------------------|-----------------|--------|--------|
| 1 轮 | ~1.0 | ~1.0 | **1.0000** ✅ | 小规模 |
| 2 轮 | ~1.0 | ~1.0 | **0.9995** ✅ | 小规模 |
| 3 轮 | — | — | **0.9745** ✅ | 小规模 |
| 5 轮 | 0.911 | 0.929 | 未做 | — |
| **6 轮** | 0.758 | **0.788** | **0.7882** ✅ | 10⁷ 样本 / 200 epoch |
| **7 轮** | 0.591 | **0.616** | **0.6101** ✅ | 10⁷ 样本 / 132 epoch |
| 8 轮 | 0.512 | 0.514 | 未做 | — |

> 7 轮说明：峰值出现在第 13 个 epoch（0.6101），之后过拟合缓降。最佳权重已按"早停"逻辑保存（`net7r.pt`）。论文 7/8 轮使用单块网络（depth=1），本复现沿用了 6 轮的 depth=10，容量偏大是过拟合的主因。

### 复现进度总览

| 环节 | 状态 |
|---|---|
| SPECK32/64 加密实现（官方向量验证） | ✅ 完成 |
| GPU 向量化数据生成（10⁷ 样本 / 0.7 秒） | ✅ 完成 |
| 1 / 2 / 3 轮区分器 | ✅ 完成 |
| 6 轮区分器（论文级配置） | ✅ **0.7882** |
| 7 轮区分器 | ✅ 0.6101 |
| 8 轮密钥恢复攻击（6 轮区分器） | ✅ 第一名率 70% |
| **9 轮密钥恢复攻击（7 轮区分器）** | ✅ **平均排名 41.8**（论文 52.1） |
| 5 / 8 轮区分器 | ⬜ 未做 |
| 11 轮攻击（中性比特 + 贝叶斯搜索） | ⬜ 未做 |
...
# 5. 神经密钥恢复攻击
python key_recovery.py                                        # 默认：6 轮区分器攻击 8 轮
python key_recovery.py --rounds 9 --net-rounds 7 --pairs 64 --trials 20   # 7 轮区分器攻击 9 轮（对照论文表 3）
